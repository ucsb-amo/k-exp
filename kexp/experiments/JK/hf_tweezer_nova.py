...
class mag_trap(EnvExperiment, Base):
    ...
    def prepare(self):
        ...
        self.xvar('t_tof',np.linspace(600.,3500.,7)*1.e-6)

        # self.xvar('dumy',[0]*3)
        # self.xvar('a1freq',np.linspace(75.75.5,10)*1.e6)
        self.p.i_hf_tweezer_tof_current = 173.
        # self.xvar('i_hf_tweezer_tof_current',np.linspace(176.1,173.,10))

        self.p.v_pd_hf_tweezer_1064_rampdown2_end = .125

        self.p.t_tweezer_hold = 3.e-3

        self.p.frequency_tweezer_list = [76.e6,75.4e6]


        a_list = [.16,.15]
        self.p.amp_tweezer_list = a_list      

        self.p.hf_imaging_detuning = -528.e6
        # self.xvar('hf_imaging_detuning', np.linspace(-550.,-510.,7)*1.e6)
    ...
    def scan_kernel(self):
        ...
        # self.set_high_field_imaging(i_outer=self.p.i_hf_tweezer_tof_current)
        ...
        self.outer_coil.ramp_supply(t=50.e-3,
                             ...
                             i_end=self.p.i_hf_tweezer_tof_current)

        #activate current PID and then ramp scattering length
        # self.outer_coil.start_pid(i_pid=self.p.i_hf_tweezer_tof_current)
