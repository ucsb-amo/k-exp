...
class rabi_posterior_pulse_train(EnvExperiment, Base):
    ...
    the Bloch vector survives across the train and pulse i has accumulated
    i*theta of rotation -- the host-side posterior
    (kexp.analysis.RabiPosterior) sharpens far faster than the same number of
    independent single-pulse shots would allow.
    ...
    def prepare(self):
        ...
        # Every pulse in the train uses the same duration. Recorded per pulse
        # anyway so the analysis never has to assume it.
