...
class dds(EnvExperiment):
    def prepare(self):
        ...
        self.dds0 = DDS(2,3,1.e6,0.6,device_db=device_db,dac_device=self.zotino0)
        self.dds0.get_devices(self)

        self.dds1 = DDS(4,2,1.e6,0.6,device_db=device_db,dac_device=self.zotino0)
    ...
    def run(self):
        ...
        for _ in range(200):
            ...
            with parallel:
                ...
                self.dds0.set_dds(frequency=200e6,
                                t_phase_origin_mu=t0,
                                phase=0.,
                                init=True)
                self.dds1.set_dds(frequency=200e6,
                                    t_phase_origin_mu=t0,
                                    phase=0.,
                                    init=True)
            ...
            # at_mu(ts)
            # delay_mu( (104+1256) & ~7 )
            # at_mu(self.dds0._t_last_change_mu)
            at_mu(self.dds1._t_last_change_mu)

            self.ttl1.on()
            delay_mu(1000 & ~7)
            self.ttl1.off()

            delay_mu(np.int64(0.05*1.e9) & ~7)

        aprint(self.dds0._t_last_change_mu - self.dds1._t_last_change_mu)
    ...
    def fast_set(self):
        ...
        ftw0 = self.dds0.dds_device.frequency_to_ftw(0.e6)
        ftw1 = self.dds1.dds_device.frequency_to_ftw(0.e6)
