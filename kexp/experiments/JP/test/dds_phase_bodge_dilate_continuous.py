...
class dds(EnvExperiment):
    def prepare(self):
        ...
        self.dt = T / N
        f0 = 30.e6
        self.f = f0 + f0 * np.linspace(-0.25,0.25,N)
    ...
    def run(self):
        ...
        while True:
            ...
            self.dds0.set_dds(frequency=self.f[0], amplitude=0.5, init=True,
                    t_phase_origin_mu=t_pulse_start+3)
            self.dds1.set_dds(frequency=self.f[i], amplitude=0.5, init=True,
