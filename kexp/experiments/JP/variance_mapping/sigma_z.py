...
class sigma_z(EnvExperiment, Base):
    ...
    def prepare(self):
        ...
        self.p.amp_imaging_weak = 0.1
        self.p.t_weak_img_pulse = 6.e-6
        self.p.amp_imaging_strong = 1.9
        self.p.t_strong_img_pulse = 4.e-6
    ...
    def scan_kernel(self):
        ...
        self.imaging.set_power(self.p.amp_imaging_weak)
        ...
        if self.p.do_weak_measurement:
            self.imaging.pulse(self.p.t_weak_img_pulse)
        else:
            delay(self.p.t_weak_img_pulse)
        self.imaging.set_power(self.p.amp_imaging_strong)

        self.raman.set_phase(relative_phase=np.pi/8)
        self.raman.pulse(self.p.t_raman_pi_pulse/2)

        delay(100.e-6)

        self.integrated_imaging_pulse(self.data.apd, t=self.p.t_strong_img_pulse)
