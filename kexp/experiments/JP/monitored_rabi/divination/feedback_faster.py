...
class feedback(EnvExperiment, Base, Feedback):
    ...
    def prepare(self):
        ...
        self.p.t_raman_pulse = 11 * self.p.t_raman_pi_pulse / 7
        self.p.t_raman_pulse_ideal = self.p.t_raman_pulse

        self.p.amp_imaging = 0.41
        self.p.t_img_pulse = 5.e-6
        self.p.frequency_lightshift = 20.34e3
        # self.xvar('frequency_lightshift', self.p.frequency_lightshift * np.linspace(1.,2.5,5))
        self.p.v_apd_all_up = -0.18379687 
        self.p.v_apd_all_down = -0.22533854
        ...
        self.N_pulses = len(rand_list) # number of steps of evolution
