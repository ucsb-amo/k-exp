...
class trap_frequency(EnvExperiment, Base):
    ...
    def prepare(self):
        ...
        self.xvar('f_tweezer_mod',np.linspace(0.59,0.85,7)*1.e3) # use awg to position modulate second aod
        # self.p.f_tweezer_mod = 500.
        # self.p.f_tweezer_mod = 3.6e3

        self.p.x_tweezer_mod_amp = .1e-6 # ~51kHz mod depth on AOD tone (2025-05-15)
