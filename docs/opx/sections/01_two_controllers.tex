...
\begin{itemize}[nosep]
  \item The OPX \emph{drives} the 80/150 AOs with sticky CW tones latched
    ...
    split, section~\ref{sec:program:config}) and at the same amplitudes:
    $\sqrt{f}\times$ the dds defaults, $f$ = \code{fraction\_power\_raman}, as
    \code{RamanBeamPair.set} writes them (the latch plays at
    \code{amp(}$\sqrt{f}$\code{)}; since 2026-09-25). The routing switch
